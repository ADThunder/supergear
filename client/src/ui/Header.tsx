            ...
            {filteredProducts.length > 0 ? (
              ...
              <div className="py-10 bg-gray-50 w-full flex items-center justify-center border border-gray-600 rounded-md">
                <p className="text-xl font-normal">
                  Nothing matches with search keywords{" "}
        ...
        <Container className="py-2 max-w-4xl flex items-center gap-5 justify-between">
          ...
          {bottomNavigation.map(({ title, link }) => (
            <Link
              to={link}
              key={title}
              className="uppercase md:inline-flex text-sm font-semibold text-whiteText/90 hover:text-whiteText duration-200 relative overflow-hidden group cursor-pointer"
            >
              {title}
